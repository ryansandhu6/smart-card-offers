...
export default async function ReviewPage() {
  ...
  const { data: pendingRaw, error: e1 } = await supabaseAdmin
    .from('card_offers')
    .select('id, card_id, headline, points_value, cashback_value, spend_requirement, spend_timeframe_days, is_monthly_bonus, monthly_points_value, monthly_spend_requirement, bonus_months, offer_type, is_limited_time, expires_at, source_priority, source_name, review_status, is_active, scraped_at, credit_cards!inner(name, slug)')
  ...
  type ActiveOfferRaw = { id: string; card_id: string; headline: string; points_value: number | null; cashback_value: number | null; spend_requirement: number | null; spend_timeframe_days: number | null; is_monthly_bonus: boolean; monthly_points_value: number | null; monthly_spend_requirement: number | null; bonus_months: number | null; offer_type: string; is_limited_time: boolean; expires_at: string | null; source_priority: number; source_name: string | null; review_status: string; is_active: boolean; scraped_at: string }
  ...
  const [{ data: activeRaw }, { data: cardDetails }] = await Promise.all([
    pendingCardIds.length
      ? supabaseAdmin
          .from('card_offers')
          .select('id, card_id, headline, points_value, cashback_value, spend_requirement, spend_timeframe_days, is_monthly_bonus, monthly_points_value, monthly_spend_requirement, bonus_months, offer_type, is_limited_time, expires_at, source_priority, source_name, review_status, is_active, scraped_at')
          .eq('is_active', true)
